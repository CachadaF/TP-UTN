CREATE PROCEDURE [LOS_NULL].[BORRAR_FK_Y_TABLE] 
...
where s.name = N'LOS_NULL')
	...
	FETCH CURSOR_BORRADO_FK INTO @NOMBRE_FK, @NOMBRE_TABLA
		WHILE (@@FETCH_STATUS = 0)
			...
			exec('ALTER TABLE LOS_NULL.'+@NOMBRE_TABLA+' DROP CONSTRAINT '+@NOMBRE_FK)
...
WHERE S.name = 'LOS_NULL')
	...
	FETCH CURSOR_BORRADO_TABLAS INTO @NOMBRE_TABLA
		WHILE (@@FETCH_STATUS = 0)
			...
			exec('DROP TABLE LOS_NULL.'+@NOMBRE_TABLA)
...
WHERE S.name = N'LOS_NULL' AND ( O.type = 'FN' OR O.type='P')
)
	...
	FETCH CURSOR_BORRADO_FUNCIONES INTO @NOMBRE_TABLA, @NOMBRE_FK
		WHILE (@@FETCH_STATUS = 0)
			...
			IF @NOMBRE_FK = 'FN' 
			BEGIN
				exec('DROP FUNCTION LOS_NULL.'+@NOMBRE_TABLA)
			END
			ELSE
			BEGIN
				exec('DROP PROCEDURE LOS_NULL.'+@NOMBRE_TABLA)
			END
...
END
GO
EXEC LOS_NULL.BORRAR_FK_Y_TABLE
GO
